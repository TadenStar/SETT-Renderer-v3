...
class MainWindow(QMainWindow):
    ...
    def _build_central(self) -> None:
        ...
        self.render_button = QPushButton("Рендер", top)
    ...
    def refresh_blender_status(self) -> None:
        ...
        if status.ok:
            ...
            self.render_button.setToolTip("Запустить рендер (появится в M2)")
            self.blender_label.setText(f"Blender: {status.path}")
        else:
            self.banner.set_message(f"Blender не настроен: {status.reason}")
            self.banner.show()
            self.render_button.setEnabled(False)
            self.render_button.setToolTip(status.reason)
            self.blender_label.setText("Blender не настроен")
